...
- `cd denarixx && npx tsx tests/embeddedRuntime.integration.test.ts` — Embedded Runtime integration tests: adapter factory, ONNX provider, Linux driver checks, embedded pipeline (135/135) — no physical device required
...
- `cd denarixx && npm run check:onnx` — Check onnxruntime-node availability + model path + model load (hardware readiness check)
- `cd denarixx && npm run validate:prototype` — Full quality gate: type-check + embedded integration tests
- `cd denarixx && npm run validate:embedded-pipeline` — Alias for validate:prototype (type-check + embedded integration tests)
