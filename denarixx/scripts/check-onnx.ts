...
async function main(): Promise<void> {
  ...
  console.log('\n[4] Test-frame execution (synthetic model-sized RGB frame)');
  ...
  if (!session || !ort) {
    ...
    try {
      ...
      if (inputNames.length === 0) {
        ...
        INFO(`Using synthetic test frame shape: [1, ${C}, ${H}, ${W}]`);
...
main().catch((error: unknown) => {
  ...
  console.error("ONNX compatibility check failed unexpectedly:");
